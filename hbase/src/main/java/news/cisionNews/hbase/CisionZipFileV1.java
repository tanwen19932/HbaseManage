...
public class CisionZipFileV1 {
	static Properties pro = null;
	static String filePath = CisionZipFileV1.class.getResource("/").getPath() + "cisionZip.properties";
	...
	public static void insertZipInputStream(ZipInputStream zis) throws Exception {
		...
		while ((zipEntry = zis.getNextEntry()) != null) {
			...
			try {
				...
				HTable hTable = new HTable(HbaseUtil.getConf(), "NewsArticleBE2");
				hTable.put(NewsUtil.genPuts(newsList));
				long tempTotal = totalCount.addAndGet(newsList.size());
				System.out.println(fileName + "  ++++ 解压完毕   共有 " + (currentFileCount + newsList.size()));

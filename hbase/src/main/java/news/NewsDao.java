...
public class NewsDao {
	...
	static final String table_log_name = "NewsArticleBE2";
	protected static final int tableN = 1;
	static final int threadN = 1;
	...
	public void Insert(List<News> newsList){
		...
		try {
			int temp = random.nextInt(tableN);
			synchronized(wTableLog[temp]){
				wTableLog[temp].put(putList);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void Insert(News news) {
		try {
			wTableLog[random.nextInt(tableN)].put(genPut(news));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	private Put genPut( News news){
		byte[] rowkey;
		if (news.getId() == null) {
...
//			return resultList;
	...
	public News getByThreadN(List<String> rowkeyList , int threadN) {
		try {
			...
			ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(threadN,
					factory);
...
//		File file = new File("/home/wwwroot/default/" + "count" + sdf.format(endDate) + ".txt");
//		Map<String, Integer> sourceCountTemp = new HashMap<>();
//		try {
//			long beginMills = System.currentTimeMillis();
//			for (int i = 0; i < 100; i++) {
...
//				rs.close();
//			}
//			System.out.println("Hbase time spend : " + (System.currentTimeMillis() - beginMills) / 1000);
//			addCount(sourceCount, sourceCountTemp);

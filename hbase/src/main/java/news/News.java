...
public class News {
    ...
    public News(byte[] id, byte[] mediaType, byte[] mediaTname, byte[] titleSrc, byte[] pubdate, byte[] textSrc,
                ...
                byte[] created, byte[] updated, byte[] isOriginal, byte[] view, byte[] url, byte[] docLength, byte[] transFromM,
                byte[] pv, byte[] isHome, byte[] isPicture, byte[] comeFrom, byte[] comeFromDb, byte[] userTag ) {

        if(id != null) this.id = Bytes.toString(id);
        if(mediaType != null) this.mediaType = Integer.parseInt( Bytes.toString(mediaType) );
        if(mediaTname != null) this.mediaTname = Bytes.toString(mediaTname);

        if(titleSrc != null) this.titleSrc = Bytes.toString(titleSrc);
        if(pubdate != null) this.pubdate = Bytes.toString(pubdate) ;
        if(textSrc != null) this.textSrc = Bytes.toString(textSrc);

        if( websiteId != null) this.websiteId = Bytes.toString(websiteId);
        if( mediaNameSrc != null) this.mediaNameSrc = Bytes.toString(mediaNameSrc);
        if( mediaNameZh != null) this.mediaNameZh = Bytes.toString(mediaNameZh);
        if( mediaNameEn != null) this.mediaNameEn = Bytes.toString(mediaNameEn);
        if( mediaLevel != null) this.mediaLevel = Integer.parseInt( Bytes.toString(mediaLevel) );

        if( countryNameZh != null) this.countryNameZh = Bytes.toString(countryNameZh);
        if( countryNameEn != null) this.countryNameEn = Bytes.toString(countryNameEn);
        if( provinceNameZh != null) this.provinceNameZh = Bytes.toString(provinceNameZh);
        if( provinceNameEn != null) this.provinceNameEn = Bytes.toString(provinceNameEn);
        if( districtNameZh != null) this.districtNameZh = Bytes.toString(districtNameZh);
        if( districtNameEn != null) this.districtNameEn = Bytes.toString(districtNameEn);

        if( languageCode != null) this.languageCode = Bytes.toString(languageCode);
        if( languageTname != null) this.languageTname = Bytes.toString(languageTname) ;

        if( author != null) this.author = Bytes.toString(author);
        if( created != null) this.created = Bytes.toString(created) ;
        if( updated != null) this.updated = Bytes.toString(updated) ;
        if( isOriginal != null) this.isOriginal = Boolean.parseBoolean( Bytes.toString(isOriginal) );

        if( view != null) this.view = Integer.parseInt( Bytes.toString(view) );
        if( url != null) this.url = Bytes.toString(url);
        if( docLength != null) this.docLength = Integer.parseInt( Bytes.toString(docLength) );

        if( transFromM != null) this.transFromM = Bytes.toString(transFromM);
        if( pv != null) this.pv = Integer.parseInt( Bytes.toString(pv) );
        if( isHome != null) this.isHome = Boolean.parseBoolean( Bytes.toString(isHome) );
        if( isPicture != null) this.isPicture = Boolean.parseBoolean( Bytes.toString(isPicture) );
        if( comeFrom != null) this.comeFrom =  Bytes.toString(comeFrom) ;
        if( comeFromDb != null) this.comeFromDb =  Bytes.toString(comeFromDb) ;
        if( userTag != null) this.userTag =  Bytes.toString(userTag) ;
    ...
    public String toString() {
        ...
        return id + "\t" + mediaType +"\t" +mediaTname +"\t" +titleSrc+"\t"
                +pubdate+"\t" +textSrc.substring(0,10)+"...\t" +websiteId+"\t" +
                mediaNameSrc+"\t" +mediaNameZh+"\t" +mediaNameEn+"\t" +countryNameZh+"\t" +countryNameEn+
                "\t" +provinceNameZh+"\t" +provinceNameEn +"\t" + districtNameZh +"\t" +districtNameEn +
                languageCode+ "\t" + languageTname + "\t" + author +"\t" +created +"\t" +
                "\t" +updated +"\t" + isOriginal +"\t" + view +"\t" + url +"\t" + comeFrom +"\t" +comeFromDb +"..";
    }
}
